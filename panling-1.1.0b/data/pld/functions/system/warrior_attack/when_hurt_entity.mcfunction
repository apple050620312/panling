...
#如果玩家手中是axe4,怪物正在流血，追擊
execute if entity @s[nbt={SelectedItem:{tag:{id:"panling:axe4",act:1}}}] unless score @s weapon_skill_axe4_5ticks matches 0.. if score @e[tag=warrior_tag,limit=1] blooded matches 1 run function pld:system/warrior_attack/weapon_skills/axe4/pursuit/main
...
#武器技-sword6
execute if entity @s[nbt={SelectedItem:{tag:{id:"panling:sword6",act:1}}}] run function pld:system/warrior_attack/weapon_skills/sword6/when_hurt_entity
