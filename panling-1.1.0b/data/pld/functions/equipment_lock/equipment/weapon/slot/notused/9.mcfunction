#複製物品
data modify block ~ 255 ~ Items[0].id set from entity @s Inventory[{Slot:8b}].id
data modify block ~ 255 ~ Items[0].tag set from entity @s Inventory[{Slot:8b}].tag

#特殊加工
execute as @s[scores={job=0}] run function pld:system/weaponlock/unlock/weapon/0
execute as @s[scores={job=1}] run function pld:system/weaponlock/unlock/weapon/1

#通用加工
#附靈屬性
execute if entity @s[scores={success=1}] run function #pld:weapon_unlock/enchant
#激活標記
execute if entity @s[scores={success=1}] run data remove block ~ 255 ~ Items[0].tag.display.Lore[-1]
execute if entity @s[scores={success=1}] run data modify block ~ 255 ~ Items[0].tag.display.Lore append value '{"translate":"pl.lore.weaponinfo2b"}'
execute if entity @s[scores={success=1}] run execute if data block ~ 255 ~ Items[{tag:{act:0}}] run data modify block ~ 255 ~ Items[{tag:{act:0}}].tag.act set value 1

#覆蓋
#loot replace entity @s hotbar.8 1 mine ~ 255 ~ minecraft:diamond_pickaxe{isShulkerMarker:1b}
item replace entity @s hotbar.8 from block ~ 255 ~ container.0
scoreboard players set @s success 0