#複製物品
data modify block ~ 255 ~ Items[0].id set from entity @s Inventory[{Slot:103b}].id
data modify block ~ 255 ~ Items[0].tag set from entity @s Inventory[{Slot:103b}].tag

#特殊加工
#execute if block ~ 255 ~ chest{Items:[{tag:{act:1,modify:1}}]} run data modify block ~ 255 ~ Items[{tag:{act:1,modify:1}}].tag.AttributeModifiers set value [{Slot:"mainhand",UUID:[I;1,2,3,4],Operation:0,AttributeName:"minecraft:generic.armor",Amount:0}]

#基礎屬性加工
execute as @s[scores={job=0}] run function #pld:armor_unlock/0/head
execute as @s[scores={job=1}] run function #pld:armor_unlock/1/head
execute as @s[scores={job=2}] run function #pld:armor_unlock/2/head
execute as @s[nbt={Inventory:[{Slot:103b,tag:{limit:3}}]}] run function #pld:armor_unlock/3/head
#通用加工
#附靈屬性
...
#item entity @s armor.head copy block ~ 255 ~ container.0
