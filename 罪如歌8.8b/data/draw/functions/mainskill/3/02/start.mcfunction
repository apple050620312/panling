...
execute positioned 655 12 -1920 run function dlc:dz/weapon/sins/20/4
...
execute if score #dlc draw.bossskill.protect matches 0 run effect give @e[tag=draw_boss] resistance 30 3

tellraw @a[scores={dlc_ins=5}] [{"text":"負罪者繪出","color": "aqua"},{"text":"澐水破千軍","color": "red"},{"text":"進行戰鬥","color": "aqua"}]
tellraw @a[scores={dlc_ins=5}] [{"text":"澐水破千軍","color": "red"},{"text":"會將受到的傷害儲存起來，待到[敵]層數充滿時釋放。","color": "aqua"}]
execute if score #dlc draw.bossskill.protect matches 0 run tellraw @a[scores={dlc_ins=5}] [{"text":"負罪者激發神器潛力，負罪者獲得抗性提升4持續30秒","color": "red"}]