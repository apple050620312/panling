...
execute as @e[tag=draw_boss] run attribute @s generic.movement_speed modifier add 2025-6-9-0-1 "boss碎夢移動速度加成" 0.5 multiply_base
execute if score #dlc draw.bossskill.protect matches 0 as @e[tag=draw_boss] run attribute @s generic.attack_damage modifier add 2025-6-9-0-1 "boss碎夢近戰傷害加成" 1 multiply_base

tellraw @a[scores={dlc_ins=5}] [{"text":"負罪者繪出","color": "aqua"},{"text":"碎夢之斧","color": "red"},{"text":"進行戰鬥","color": "aqua"}]
tellraw @a[scores={dlc_ins=5}] [{"text":"碎夢之斧","color": "red"},{"text":"：持有此武器時，負罪者持續獲得抗性提升2，且移速提升50%","color": "aqua"}]
execute if score #dlc draw.bossskill.protect matches 0 run tellraw @a[scores={dlc_ins=5}] [{"text":"負罪者激發神器潛力，負罪者近戰傷害提升100%","color": "red"}]