...
kill @e[type=item,limit=1,sort=nearest,nbt={Item:{tag:{fairy:1}}}]
#loot spawn ~ ~ ~ loot fire:goldchest
loot spawn ~ ~ ~ loot fairy:goldchest
execute as @e[limit=1,type=item,sort=nearest,nbt={Item:{tag:{fairy:1,chest:3}}}] run function fairy:chest/judge
